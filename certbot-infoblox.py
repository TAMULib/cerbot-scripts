...
INFOBLOX_URL = "https://my.infoblox.example.com/wapi/v2.7/" # Must include https:// and path to whatever WAPI version we want to use. Script was tested on v2.7.
INFOBLOX_USER = "" 
INFOBLOX_PASSWORD = ""
...
def set_acme_record(record_name, challenge):
    ...
    record_url = INFOBLOX_URL + "record:txt?_return_fields%2B=name,text&_return_as_object=1"
    print('POSTing challenge data to ' + record_name + ', data: ' + challenge)
    return requests.post(record_url, json=record_data, auth=(INFOBLOX_USER, INFOBLOX_PASSWORD)).status_code

# OS Agnostic. query_url, query_data, query_results, and record_url all need to be tweaked to match the format of your DNS provider's API.
def delete_acme_record(record_name):
    query_url = INFOBLOX_URL + "record:txt?_return_fields%2B=name,text&_return_as_object=1"
    query_data = {
        "name": record_name,
        "view": "Internet"
    }
    print('Sending query to Infoblox')
    query_response = requests.get(query_url, json=query_data, auth=(INFOBLOX_USER, INFOBLOX_PASSWORD))
    if query_response.status_code < 200 or query_response.status_code > 299:
        print('Infoblox query failed!')
        print('Returned status code ' + str(query_response.status_code) + ' and json data:\n' + str(query_response.json()), file=sys.stderr)
        return query_response.status_code
    query_results = query_response.json()["result"]
    if len(query_results) > 0:
        print('DELETEing record ' + record_name)
        for i in range(0, len(query_results)):
            record_url = INFOBLOX_URL + query_results[i]["_ref"] + "?_return_as_object=1"
            del_response = requests.delete(record_url, json=query_data, auth=(INFOBLOX_USER, INFOBLOX_PASSWORD))
            if del_response.status_code < 200 or del_response.status_code > 299:
                print("DELETE request for " + record_name + " failed with code " + str(del_response.status_code) + " and json data:\n" + str(del_response.json()), file=sys.stderr)
...
for domains_raw in domain_list:
    ...
    if check_cert_expiration(CERT_PATH, REPLACE_AFTER_SECONDS):
        ...
        while certbot_process.poll() is None:
            ...
            if 'with the following value:' in last_line:
                ...
                if status_code < 200 or status_code > 299:
                    print('Infoblox GET or PUT request returned with status code ' + str(status_code) + '!', file=sys.stderr)
                    failed_entry = [domains, "The GET or PUT request to Infoblox returned with status code " + str(status_code)]
                    failed_list.append(failed_entry)
                    failed_flag = True
                    certbot_process.terminate()
                    break
                record_list.append(record_name)
                print('Infoblox entry for _acme-challenge successfully updated. Waiting ' + str(DELAY_SECONDS) + ' to continue.')
        ...
        for rec in list(set(record_list)):
            ...
            if del_ret < 200 or del_ret > 299:
                msg = EmailMessage()
                msg['Subject'] = "Error on Certbot Cleanup"
                msg['To'] = CONTACT_EMAIL
                msg['From'] = FROM_EMAIL
                msg.set_content("Unable to delete the leftover ACME records for " + rec + ", this will need to be deleted manually in Infoblox. Status Code: " + str(del_ret))
